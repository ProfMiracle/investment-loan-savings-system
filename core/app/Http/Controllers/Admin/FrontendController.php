...
class FrontendController extends Controller
{
    ...
    public function sectionAbout()
    {
        ...
        if ($basic->active_template == 'basic') {
            ...
            return view('admin.frontend.section.about-minimul', compact('page_title', 'post'));
        }
        abort(404);
    ...
    public function sectionHomeContent()
    {
        ...
        if ($basic->active_template == 'basic') {
            $post = Frontend::where('data_keys', 'homecontent')->firstOrFail();
            return view('admin.frontend.section.home-basic-content', compact('page_title', 'post'));
        } else if ($basic->active_template == 'minimul') {
            $post = Frontend::where('data_keys', 'homecontent2')->firstOrFail();
            return view('admin.frontend.section.home-minimul-content', compact('page_title', 'post'));
        }
    ...
    public function sectionHomeContentUpdate(Request $request, $id)
    {
        ...
        if ($basic->active_template == 'basic') {
            ...
            $data = Frontend::where('id', $id)->where('data_keys', 'homecontent')->firstOrFail();

            $in['title'] = $request->title;
            $in['featured_title'] = $request->featured_title;
            $in['site_information'] = $request->site_information;
            $in['transaction_title'] = $request->transaction_title;
            $in['details'] = $request->details;

            $in['can_see_featured'] = ($request->can_see_featured == 'on') ? 1 : 0;
            $in['can_see_trx'] = ($request->can_see_trx == 'on') ? 1 : 0;
            $in['can_see_map'] = ($request->can_see_map == 'on') ? 1 : 0;
            $in['can_see_info'] = ($request->can_see_info == 'on') ? 1 : 0;
            $in['we_accept'] = ($request->we_accept == 'on') ? 1 : 0;
            $in['subscription_form'] = ($request->subscription_form == 'on') ? 1 : 0;

            if ($request->hasFile('image')) {
                @unlink('assets/images/frontend/' . @$data->data_values->image);
                $image = $request->file('image');
                $filename = 'home01.' . $image->getClientOriginalExtension();
                $location = 'assets/images/frontend/' . $filename;
                Image::make($image)->save($location);
                $in['image'] = $filename;
            } else {
            ...
            if ($request->hasFile('dragon')) {
                @unlink('assets/images/frontend/' . @$data->data_values->dragon);
                $image = $request->file('dragon');
                $filename = 'dragon.' . $image->getClientOriginalExtension();
                $location = 'assets/images/frontend/' . $filename;
                Image::make($image)->save($location);
                $in['dragon'] = $filename;
            } else {
                $in['dragon'] = @$data->data_values->dragon;
            }

            if ($request->hasFile('map')) {
                @unlink('assets/images/frontend/' . @$data->data_values->map);
                $image = $request->file('map');
                $filename = 'map.' . $image->getClientOriginalExtension();
                $location = 'assets/images/frontend/' . $filename;
                Image::make($image)->save($location);
                $in['map'] = $filename;
            } else {
                $in['map'] = @$data->data_values->map;
            }
